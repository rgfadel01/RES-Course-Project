...
def plot_map_modified(network, tech_colors, threshold=10,components=["links", "stores", "generators"],
...
#         link_widths = [0]*len(n.links.query("capital_cost == 240000"))
    ...
    fig.suptitle('Installed ' + plot_item + ' capacities and transmission lines for the UK, FR, and DK',y=0.92,fontsize=15)
    
    handles = make_legend_circles_for(
        [legend_size/50,legend_size], scale=bus_size_factor, facecolor="white")
    str1 = ["    {} ".format(s) for s in (legend_size/10,legend_size)]
    labels = [x + unit_string for x in str1]
    l1 = ax.legend(handles, labels,
                   loc="upper left", bbox_to_anchor=(0.5, 0.96),
                   labelspacing=4,
                   frameon=False,
                   title='',
